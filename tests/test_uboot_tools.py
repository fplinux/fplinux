...
"""Behavioral tests for the profile-owned full U-Boot cache producer."""
...
class UbootToolsTests(unittest.TestCase):
    """Build the complete profile stage through a deliberately tiny source tree."""
    ...
    def test_failed_changed_recipe_preserves_previous_complete_output_and_receipt(self) -> None:
        ...
        self._build()
        output = self.work / "uboot"
        before = {
            name: (output / name).read_bytes()
            for name in (*uboot_tools.FULL_OUTPUTS, uboot_tools.RECEIPT_NAME)
        }
        self.defconfig.write_text(
