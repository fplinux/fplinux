...
"""Host control-flow coverage for the MicroPythonOS storage policy.
...
STORAGE = BASE_APORT / "fplinux_storage.py"
...
def load_storage_module() -> _StorageModule:
    """Load the target-neutral MicroPythonOS storage adapter."""
...
class MicroPythonOsStorageTests(unittest.TestCase):
    """Exercise storage-policy decisions without pretending to mount a card."""
...
""",
    ...
    def test_storage_adapter_hides_storage_when_mountinfo_cannot_be_read(self) -> None:
        """An unreadable mount table is treated as no mounted storage by the adapter."""
        module = load_storage_module()
        storage = module.FPLinuxStorage
    ...
    def test_storage_adapter_exposes_only_an_exact_mountinfo_match(self) -> None:
        """The adapter's reported mount point comes from the configured exact path."""
        module = load_storage_module()
        storage = module.FPLinuxStorage
