...
class SetupLifecycleTests(unittest.TestCase):
    ...
    def test_podman_image_probe_timeout_is_reported(self) -> None:
        """A stuck image lookup fails with its named boundary instead of hanging."""
        with (
            mock.patch(
                "fplinux_cli.container.subprocess.run",
                side_effect=subprocess.TimeoutExpired(["podman", "image", "exists"], 60),
            ),
            self.assertRaisesRegex(SystemExit, "Podman image lookup timed out"),
        ):
            container.image_exists("podman", "localhost/fplinux:locked")
