...
class FitImageTests(unittest.TestCase):
    """Build and reuse a FIT with explicitly supplied, pinned U-Boot tools."""
    ...
    def setUp(self) -> None:
        ...
        source_mkimage, source_dumpimage = self._declared_tools()
    ...
    def _declared_tools(self) -> tuple[Path, Path]:
        """Read test-owned tool paths instead of discovering an incidental cache."""
        names = ("FPLINUX_TEST_MKIMAGE", "FPLINUX_TEST_DUMPIMAGE")
        mkimage, dumpimage = (os.environ.get(name) for name in names)
        if not mkimage or not dumpimage:
            self.skipTest("set FPLINUX_TEST_MKIMAGE and FPLINUX_TEST_DUMPIMAGE for FIT artifacts")
        tools = (Path(mkimage), Path(dumpimage))
        invalid = [
            str(tool)
            for tool in tools
        ...
        if invalid:
            self.skipTest("declared U-Boot FIT tools are unavailable: " + ", ".join(invalid))
        for tool in tools:
            try:
                ...
                self.skipTest(f"declared U-Boot FIT tool cannot run: {tool}: {error}")
            if result.returncode != 0:
                self.skipTest(f"declared U-Boot FIT tool self-test failed: {tool}")
        return tools
    ...
    def _copy_declared_tool(self, source: Path) -> Path:
        ...
        try:
            ...
            self.skipTest(f"cannot copy declared U-Boot FIT tool {source}: {error}")
        return destination
