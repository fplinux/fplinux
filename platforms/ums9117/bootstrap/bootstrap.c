...
{
	if (snapshot != NULL) {
		snapshot->eb0_before = reg_read(AON_APB_EB0);
		snapshot->rtc_before = reg_read(AON_APB_RTC_EB);
		snapshot->clk_before = reg_read(AON_APB_CLK_EB0);
	}
	...
	if (snapshot != NULL) {
		snapshot->eb0_after = reg_read(AON_APB_EB0);
		snapshot->rtc_after = reg_read(AON_APB_RTC_EB);
		snapshot->clk_after = reg_read(AON_APB_CLK_EB0);
	}
}
...
int ums9117_bootstrap_probe_timer(struct ums9117_bootstrap_timer_result *result)
{
	...
	result->ctl_before = reg_read(AON_TIMER_BASE + TIMER_CTL);
	reg_write(AON_TIMER_BASE + TIMER_CTL,
		  result->ctl_before & ~TIMER_CTL_ENABLE);
	...
	result->ctl =
		(result->ctl_before & ~(TIMER_CTL_PERIOD | TIMER_CTL_64BIT)) |
		TIMER_CTL_ENABLE;

	result->syscnt_before = reg_read(AP_SYSCNT);
	reg_write(AON_TIMER_BASE + TIMER_CTL, result->ctl);
	result->polls = 0;
	do {
		result->int_status = reg_read(AON_TIMER_BASE + TIMER_INT);
		result->syscnt_after = reg_read(AP_SYSCNT);
	} while (!(result->int_status & (TIMER_INT_RAW | TIMER_INT_MASK)) &&
		 (uint32_t)(result->syscnt_after - result->syscnt_before) <
			 50 &&
		 ++result->polls < 5000000ul);

	result->value = reg_read(AON_TIMER_BASE + TIMER_VALUE_LO);
	result->shadow = reg_read(AON_TIMER_BASE + TIMER_SHDW_LO);
	reg_write(AON_TIMER_BASE + TIMER_CTL,
		  reg_read(AON_TIMER_BASE + TIMER_CTL) & ~TIMER_CTL_ENABLE);
	reg_write(AON_TIMER_BASE + TIMER_INT,
		  result->int_status | TIMER_INT_CLR);
	reg_write(AON_TIMER_BASE + TIMER_INT, 0);
	__asm__ volatile("dsb sy" : : : "memory");

	return result->syscnt_after != result->syscnt_before &&
	       (result->int_status & (TIMER_INT_RAW | TIMER_INT_MASK));
}
...
uint32_t ums9117_bootstrap_quiesce_usb_dma_channel(unsigned channel)
{
	...
	return (intr & MUSB_DMA_CLR_STATUS ? UMS9117_BOOTSTRAP_DMA_CLEAR_SEEN :
					     0) |
	       (reg_read(base + MUSB_DMA_CFG) & MUSB_DMA_CHN_EN ?
			0 :
