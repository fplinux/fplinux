...
struct sc2720_auxadc {
	struct device *dev;
	struct mutex lock;
	bool faulted;
};

struct sc2720_auxadc_gates {
	bool module_enabled;
...
static int
sc2720_auxadc_finish_transaction(struct ums9117_adi_transaction *transaction,
...
static int sc2720_auxadc_read_if_ok(struct ums9117_adi_transaction *transaction,
...
static bool sc2720_auxadc_debug_idle(u16 debug)
{
	return !(debug & (SC2720_AUXADC_DEBUG_STATE_MASK |
			  SC2720_AUXADC_DEBUG_COUNTER_MASK));
}

static int sc2720_auxadc_validate_identity(u16 id_low, u16 id_high)
{
...
static int sc2720_auxadc_check_identity(void)
{
	...
	ret = sc2720_auxadc_read_if_ok(&transaction, ret, SC2720_CHIP_ID_HIGH,
				       &id_high);
	ret = sc2720_auxadc_finish_transaction(&transaction, ret, &uncertain);
	if (ret)
		return ret;

	return sc2720_auxadc_validate_identity(id_low, id_high);
}

static int sc2720_auxadc_enable_gates(struct sc2720_auxadc_session *session,
				      bool *uncertain)
{
	struct ums9117_adi_transaction transaction = {};
	u16 arm_clk_en0;
	...
	*uncertain = false;
	...
	ret = sc2720_auxadc_read_if_ok(&transaction, ret, SC2720_CHIP_ID_HIGH,
				       &id_high);
	ret = sc2720_auxadc_read_if_ok(&transaction, ret, SC2720_MODULE_EN0,
				       &module_en0);
	...
	if (!ret && (soft_rst0 & SC2720_SOFT_RST0_ADC))
		ret = -EHOSTDOWN;
	if (ret)
		return sc2720_auxadc_finish_transaction(&transaction, ret,
							uncertain);
	...
	return sc2720_auxadc_finish_transaction(&transaction, ret, uncertain);
}

static int sc2720_auxadc_snapshot_bank(struct sc2720_auxadc_session *session,
				       bool *uncertain)
{
	struct ums9117_adi_transaction transaction = {};
	struct sc2720_auxadc_bank *bank = &session->bank;
	u16 debug;
	...
	*uncertain = false;
	...
	ret = sc2720_auxadc_read_if_ok(&transaction, ret, SC2720_AUXADC_CTRL,
				       &bank->ctrl);
	ret = sc2720_auxadc_read_if_ok(
		&transaction, ret, SC2720_AUXADC_SW_CH_CFG, &bank->sw_ch_cfg);
	ret = sc2720_auxadc_read_if_ok(&transaction, ret, SC2720_AUXADC_INT_EN,
				       &bank->int_en);
	ret = sc2720_auxadc_read_if_ok(&transaction, ret, SC2720_AUXADC_INT_STS,
				       &int_sts);
	ret = sc2720_auxadc_read_if_ok(&transaction, ret, SC2720_AUXADC_INT_RAW,
				       &int_raw);
	ret = sc2720_auxadc_read_if_ok(&transaction, ret, SC2720_AUXADC_DEBUG,
				       &debug);
	ret = sc2720_auxadc_read_if_ok(&transaction, ret,
	...
	if (version != SC2720_AUXADC_EXPECTED_VERSION ||
	    ...
	    !sc2720_auxadc_debug_idle(debug)) {
		*uncertain = true;
		return -EPROTO;
	}
...
static int sc2720_auxadc_clear_raw(bool *uncertain)
{
	struct ums9117_adi_transaction transaction = {};
	u16 ctrl;
	u16 debug;
	...
	*uncertain = false;
	...
	if ((ctrl & SC2720_AUXADC_CTRL_RUN) || (int_sts & SC2720_AUXADC_INT) ||
	    (int_raw & SC2720_AUXADC_INT) || !sc2720_auxadc_debug_idle(debug)) {
		*uncertain = true;
		return -EPROTO;
	}
...
static int sc2720_auxadc_start_conversion(struct sc2720_auxadc_session *session,
					  u8 channel, bool *uncertain)
{
	...
	ret = sc2720_auxadc_clear_raw(uncertain);
	if (ret)
		return ret;
...
static int sc2720_auxadc_read_conversion(u16 *raw, bool *uncertain)
{
	ktime_t deadline;
	...
	*uncertain = false;
	...
	for (;;) {
		...
		ret = sc2720_auxadc_finish_transaction(&transaction, ret,
						       uncertain);
		if (ret)
			return ret;
		...
		if (!(ctrl & SC2720_AUXADC_CTRL_RUN) &&
			...
			ret = sc2720_auxadc_finish_transaction(&transaction,
							       ret, uncertain);
			if (ret)
				return ret;
			if (!(ctrl & SC2720_AUXADC_CTRL_RUN) &&
...
static int sc2720_auxadc_wait_idle(bool *uncertain)
{
	ktime_t deadline;
...
static int sc2720_auxadc_restore_bank(struct sc2720_auxadc_session *session,
				      bool *uncertain)
{
	struct ums9117_adi_transaction transaction = {};
	struct sc2720_auxadc_bank *bank = &session->bank;
	...
	ret = sc2720_auxadc_wait_idle(uncertain);
	if (ret)
		return ret;
	ret = sc2720_auxadc_clear_raw(uncertain);
	if (ret)
...
static int sc2720_auxadc_restore_gates(struct sc2720_auxadc_session *session,
				       bool *uncertain)
{
	struct ums9117_adi_transaction transaction = {};
...
static int sc2720_auxadc_sample(struct sc2720_auxadc *adc, u8 channel, int *raw)
{
	struct sc2720_auxadc_session session = {};
	bool uncertain = false;
	u16 sample;
	int cleanup_ret;
	int ret;
	...
	if (ret)
		goto finish;
	ret = sc2720_auxadc_start_conversion(&session, channel, &uncertain);
	if (ret)
		goto finish;
	ret = sc2720_auxadc_read_conversion(&sample, &uncertain);
...
static int sc2720_auxadc_read_raw(struct iio_dev *indio_dev,
				  struct iio_chan_spec const *chan, int *val,
				  int *val2, long mask)
{
	struct sc2720_auxadc *adc = iio_priv(indio_dev);
	int ret;

	switch (mask) {
	case IIO_CHAN_INFO_RAW:
		ret = sc2720_auxadc_sample(adc, chan->channel, val);
...
static const struct iio_info sc2720_auxadc_info = {
	.read_raw = sc2720_auxadc_read_raw,
};

#define SC2720_AUXADC_CHANNEL(_channel)                       \
	{                                                     \
		.type = IIO_VOLTAGE,                          \
		.indexed = 1,                                 \
		.channel = (_channel),                        \
...
static const struct iio_chan_spec sc2720_auxadc_channels[] = {
	SC2720_AUXADC_CHANNEL(0),  SC2720_AUXADC_CHANNEL(1),
	SC2720_AUXADC_CHANNEL(2),  SC2720_AUXADC_CHANNEL(4),
	SC2720_AUXADC_CHANNEL(14),
};

static int sc2720_auxadc_probe(struct platform_device *pdev)
{
	...
	indio_dev->num_channels = ARRAY_SIZE(sc2720_auxadc_channels);

	return devm_iio_device_register(&pdev->dev, indio_dev);
}

static const struct of_device_id sc2720_auxadc_of_match[] = {
	{ .compatible = "sprd,ums9117-sc2720-auxadc" },
	{},
};
MODULE_DEVICE_TABLE(of, sc2720_auxadc_of_match);

static struct platform_driver sc2720_auxadc_driver = {
	.probe = sc2720_auxadc_probe,
	.driver = {
		.name = "sc2720-auxadc-ums9117",
		.of_match_table = sc2720_auxadc_of_match,
	},
};
module_platform_driver(sc2720_auxadc_driver);

MODULE_DESCRIPTION("SC2720 direct AUXADC raw provider on UMS9117");
MODULE_LICENSE("GPL");
