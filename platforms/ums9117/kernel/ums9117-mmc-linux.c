...
/*
 * Linux MMC host for the UMS9117 SDIO0 removable microSD controller.
 *
...
static_assert(UMS9117_SDIO0_INTID == 89U);

struct ums9117_mmc_ios_trace_entry {
	u32 sequence;
	u32 clock;
	u32 power_mode;
	u32 bus_width;
...
struct ums9117_mmc_audit {
	struct ums9117_mmc_ios_trace_entry ios[UMS9117_MMC_IOS_TRACE_DEPTH];
	u32 ios_calls;
	u32 ios_trace_count;
...
struct ums9117_mmc_host {
	...
	struct ums9117_mmc_audit audit;
};

static u32 ums9117_mmc_controller_read(void *context, enum ums9117_sdio_reg reg)
{
...
static void ums9117_mmc_controller_write(void *context,
...
static void ums9117_mmc_delay_us(void *context, u32 usec)
{
	(void)context;
	udelay(usec);
}

static void ums9117_mmc_sleep_us(void *context, u32 minimum, u32 maximum)
{
	(void)context;
	if (minimum == maximum)
		maximum++;
...
static void ums9117_mmc_data_barrier(void *context)
{
	(void)context;
	dma_wmb();
}

static void ums9117_mmc_sleep_ms(void *context, u32 msec)
{
	(void)context;
	msleep(msec);
}

static int ums9117_mmc_adi_begin(void *context)
{
	struct ums9117_mmc_host *host = context;
...
static int ums9117_mmc_adi_read(void *context,
				enum ums9117_sdio_slot_analog_reg reg,
				u16 *value)
{
	struct ums9117_mmc_host *host = context;

	return ums9117_adi_read(&host->adi_transaction,
				ums9117_sdio_slot_analog_offset(reg), value);
}

static int ums9117_mmc_adi_write(void *context,
				 enum ums9117_sdio_slot_analog_reg reg,
				 u16 value)
{
	struct ums9117_mmc_host *host = context;
...
static int ums9117_mmc_adi_end(void *context)
{
	struct ums9117_mmc_host *host = context;
...
static void ums9117_mmc_record_ios(struct ums9117_mmc_host *host,
				   const struct mmc_ios *ios, int result)
{
	struct ums9117_mmc_ios_trace_entry *entry;
	unsigned long flags;
	u32 sequence;

	spin_lock_irqsave(&host->lock, flags);
	sequence = ++host->audit.ios_calls;
	entry = &host->audit.ios[(sequence - 1U) % UMS9117_MMC_IOS_TRACE_DEPTH];
	...
	entry->result = result;
	if (host->audit.ios_trace_count < UMS9117_MMC_IOS_TRACE_DEPTH)
		host->audit.ios_trace_count++;
	spin_unlock_irqrestore(&host->lock, flags);
}

static void ums9117_mmc_audit_command_locked(struct ums9117_mmc_host *host,
...
static void ums9117_mmc_audit_irq_locked(struct ums9117_mmc_host *host,
...
static bool ums9117_mmc_dma_address_valid(dma_addr_t address, size_t length)
{
...
static int ums9117_mmc_set_card_clock(struct ums9117_mmc_host *host,
				      bool enable)
{
	...
	ret = ums9117_sdio_slot_enable_ident_clock(&host->sdio,
						   &host->sdio_state, &record);
	if (ret)
		return ret;
	spin_lock_irqsave(&host->lock, flags);
...
static int ums9117_mmc_activate_platform(struct ums9117_mmc_host *host)
{
	...
	ret = ums9117_sdio_slot_activate(&host->sdio, &host->sdio_state);
	if (ret || already_active)
		return ret;
	host->audit.host_control2_after_reset = ums9117_mmc_controller_read(
	...
	host->app_cmd_arg = 0;
	control = ums9117_mmc_controller_read(host,
...
static void ums9117_mmc_restore_platform(struct ums9117_mmc_host *host)
{
	int ret;

	ret = ums9117_sdio_slot_restore_platform(
...
static bool ums9117_mmc_is_destructive_command(const struct mmc_command *cmd)
{
	switch (cmd->opcode) {
...
static int ums9117_mmc_response_type(const struct mmc_command *cmd,
...
static void ums9117_mmc_unmap_data(struct ums9117_mmc_host *host)
{
...
static int ums9117_mmc_prepare_data(struct ums9117_mmc_host *host,
				    struct mmc_command *cmd)
{
	...
	for_each_sg(data->sg, sg, mapped, i) {
		...
		if (!length || length > host->mmc->max_seg_size ||
		    !ums9117_mmc_dma_address_valid(address, length)) {
...
static void ums9117_mmc_width_fail_closed(struct ums9117_mmc_host *host,
					  const char *reason, int error)
{
	...
	ums9117_mmc_controller_write(
		host, UMS9117_SDIO_REG_INTERRUPT_SIGNAL_ENABLE, 0);
	signal_after = ums9117_mmc_controller_read(
	...
	if (clock_after != clock_before)
		ums9117_mmc_controller_write(host, UMS9117_SDIO_REG_CLOCK_RESET,
					     clock_after);
	clock_after =
		ums9117_mmc_controller_read(host, UMS9117_SDIO_REG_CLOCK_RESET);
...
static void ums9117_mmc_reject_request(struct ums9117_mmc_host *host,
...
static bool ums9117_mmc_select_clock_profile(struct ums9117_mmc_host *host,
...
static int ums9117_mmc_transition_width4(struct ums9117_mmc_host *host)
{
	...
	ret = ums9117_sdio_slot_set_operational_clock(
		&host->sdio, &host->sdio_state, host->target_clock_profile,
		&record);
...
static bool ums9117_mmc_wait_quiescent(struct ums9117_mmc_host *host,
...
static void ums9117_mmc_finish_work(struct work_struct *work)
{
	struct ums9117_mmc_host *host =
		container_of(work, struct ums9117_mmc_host, finish_work);
...
static int ums9117_mmc_recover(struct ums9117_mmc_host *host)
{
...
{
	struct ums9117_mmc_host *host = container_of(
		to_delayed_work(work), struct ums9117_mmc_host, timeout_work);
	struct mmc_request *mrq;
	unsigned long flags;
...
{
	struct ums9117_mmc_host *host = data;
	struct ums9117_sdio_completion completion = { 0 };
	struct mmc_request *mrq;
	struct mmc_command *cmd;
	unsigned long flags;
	...
	if (irq != host->irq)
		return IRQ_NONE;
	status = ums9117_mmc_controller_read(host,
	...
	if (terminal) {
		...
		completion.auto_command_status = ums9117_mmc_controller_read(
			host, UMS9117_SDIO_REG_HOST_CONTROL2);
		ums9117_mmc_controller_write(
			host, UMS9117_SDIO_REG_INTERRUPT_STATUS, owned);
		completion.status_readback = ums9117_mmc_controller_read(
...
{
	struct ums9117_mmc_host *host = mmc_priv(mmc);
	...
	if (ums9117_mmc_is_destructive_command(cmd) ||
	...
	ret = ums9117_mmc_response_type(cmd, &response_type);
	if (!ret)
	...
	ret = ums9117_sdio_slot_validate_active(&host->sdio, &host->sdio_state);
	if (ret) {
		spin_unlock_irqrestore(&host->lock, flags);
		goto out_error;
	}
	if ((multi_read || multi_write) &&
	    ums9117_mmc_controller_read(host, UMS9117_SDIO_REG_HOST_CONTROL2) !=
		    UMS9117_SDIO_HOST_CTRL2_EXPECTED) {
		spin_unlock_irqrestore(&host->lock, flags);
		ret = -EPROTO;
		goto out_error;
...
static int ums9117_mmc_set_host_1bit_powered_off(struct ums9117_mmc_host *host)
{
	unsigned long flags;
	int ret = 0;
...
static void ums9117_mmc_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	case MMC_POWER_OFF:
		if (host->sdio_state.platform_active) {
			...
			if (!ret)
				ret = ums9117_sdio_slot_set_slot_power(
	...
	case MMC_POWER_UP:
		...
		ret = ums9117_mmc_activate_platform(host);
		break;
	case MMC_POWER_ON:
		if (!host->sdio_state.platform_active ||
...
out_fail:
	...
	ums9117_mmc_restore_platform(host);
out_trace:
...
static int ums9117_mmc_get_cd(struct mmc_host *mmc)
{
	...
	present =
		ums9117_sdio_slot_card_present(&host->sdio, &host->sdio_state);
	if (present < 0) {
		dev_err_ratelimited(
			host->dev,
...
static ssize_t audit_show(struct device *dev, struct device_attribute *attr,
			  char *buf)
{
	struct ums9117_mmc_host *host = dev_get_drvdata(dev);
	struct ums9117_mmc_audit audit;
	unsigned long flags;
	bool active;
	bool app_cmd_armed;
	...
	for (i = 0; i < UMS9117_MMC_IOS_TRACE_DEPTH && len < PAGE_SIZE; i++) {
		const struct ums9117_mmc_ios_trace_entry *entry = &audit.ios[i];
...
static int
ums9117_mmc_map_resource(struct platform_device *pdev,
			 const struct ums9117_sdio_resource *definition,
			 void __iomem **mapped)
{
...
static int ums9117_mmc_map_sdio_resources(struct platform_device *pdev,
					  struct ums9117_mmc_host *host)
{
	const struct ums9117_sdio_resource *definition;
	struct resource *resource;
	...
	for (index = 0; index < UMS9117_SDIO_REG_COUNT; ++index) {
		definition = ums9117_sdio_slot_controller_resource(index);
	...
	definition = ums9117_sdio_slot_adi_resource();
	resource = platform_get_resource_byname(pdev, IORESOURCE_MEM,
						definition->name);
	if (!resource || resource->start != definition->address ||
		...
		return dev_err_probe(
			...
			definition->name, definition->address,
			definition->size);
	definition = ums9117_sdio_slot_analog_resource();
	resource = platform_get_resource_byname(pdev, IORESOURCE_MEM,
						definition->name);
	if (!resource || resource->start != definition->address ||
	    resource_size(resource) != definition->size)
	...
	if (platform_get_resource(pdev, IORESOURCE_MEM,
				  UMS9117_SDIO_REG_COUNT +
					  UMS9117_SDIO_SLOT_REG_COUNT + 2U))
...
static void ums9117_mmc_init_sdio(struct ums9117_mmc_host *host)
{
	...
	host->sdio.adi_end = ums9117_mmc_adi_end;
	host->sdio.sleep_ms = ums9117_mmc_sleep_ms;
	host->sdio.controller.context = host;
	host->sdio.controller.read = ums9117_mmc_controller_read;
	host->sdio.controller.write = ums9117_mmc_controller_write;
	host->sdio.controller.time_us = ums9117_mmc_time_us;
	host->sdio.controller.delay_us = ums9117_mmc_delay_us;
	host->sdio.controller.sleep_us = ums9117_mmc_sleep_us;
	host->sdio.controller.data_barrier = ums9117_mmc_data_barrier;
}

static void ums9117_mmc_restore_card_detect(struct ums9117_mmc_host *host)
{
	int ret;

	WRITE_ONCE(host->card_detect_enabled, false);
	ret = ums9117_sdio_slot_restore_card_detect(&host->sdio,
						    &host->sdio_state);
	if (ret)
		dev_err(host->dev,
...
static int ums9117_mmc_enable_card_detect(struct ums9117_mmc_host *host)
{
	int ret;

	ret = ums9117_sdio_slot_enable_card_detect(&host->sdio,
						   &host->sdio_state);
	if (ret) {
		ums9117_mmc_restore_card_detect(host);
		return ret;
	}
...
static int ums9117_mmc_probe(struct platform_device *pdev)
{
	...
	if (hwirq != UMS9117_SDIO0_INTID || trigger != IRQ_TYPE_LEVEL_HIGH)
		return dev_err_probe(
			&pdev->dev, -EINVAL,
			"IRQ must be SDIO0 SPI%u/INTID%u level-high, got %lu type 0x%x\n",
			UMS9117_SDIO0_SPI, UMS9117_SDIO0_INTID,
	...
	ums9117_mmc_init_sdio(host);
	ret = ums9117_mmc_map_sdio_resources(pdev, host);
	if (ret)
		goto out_free_host;
	...
	if (!ums9117_mmc_dma_address_valid(host->descriptors_dma,
					   UMS9117_MMC_ADMA2_TABLE_BYTES)) {
		ret = -ERANGE;
		goto out_free_descriptor;
	}

	ret = ums9117_sdio_slot_snapshot(&host->sdio, &host->sdio_state);
	if (ret)
		goto out_free_descriptor;
	...
	ret = ums9117_mmc_enable_card_detect(host);
	if (ret)
