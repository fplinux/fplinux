...
static u32 ums9117_fb_lcm_dbi_cycles(u32 ns, u32 max)
{
	u32 cycles = DIV_ROUND_UP(UMS9117_FB_LCM_AHB_MHZ * ns, 1000);
 ...
 */
static u32 ums9117_fb_lcm_dbi_timing(const u32 ns[6])
{
	u32 rcss = ums9117_fb_lcm_dbi_cycles(ns[0], 6);
	u32 rlpw = ums9117_fb_lcm_dbi_cycles(ns[1], 14);
	u32 rhpw = ums9117_fb_lcm_dbi_cycles(ns[2], 14);
	u32 wcss = ums9117_fb_lcm_dbi_cycles(ns[3], 6);
	u32 wlpw = ums9117_fb_lcm_dbi_cycles(ns[4], 14);
	u32 whpw = ums9117_fb_lcm_dbi_cycles(ns[5], 14);
...
static int ums9117_fb_lcm_wait_idle(struct ums9117_fb *ufb)
{
	u32 status;
			       ...
			       struct platform_device *pdev)
{
	...
	packed = ums9117_fb_lcm_dbi_timing(timings);
...
static int ums9117_fb_lcm_enable(struct ums9117_fb *ufb)
{
	int ret = ums9117_fb_lcm_wait_idle(ufb);

	if (ret)
		return ret;
	writel(0, ufb->lcm + UMS9117_FB_LCM_CTRL);
	writel(1, ufb->lcm + UMS9117_FB_LCM_CS0_MODE);
	/* Keep the controller in fpdoom's generic-safe state during reset. */
	writel(0x00a50100, ufb->lcm + UMS9117_FB_LCM_CS0_TIMING);
	return ums9117_fb_lcm_wait_idle(ufb);
}

static int ums9117_fb_lcm_program_after_reset(struct ums9117_fb *ufb)
{
	int ret = ums9117_fb_lcm_wait_idle(ufb);
	...
	writel(1, ufb->lcm + UMS9117_FB_LCM_CS0_MODE);
	writel(0x00a50100, ufb->lcm + UMS9117_FB_LCM_CS0_TIMING);
	writel(0x11110000, ufb->lcm + UMS9117_FB_LCM_CTRL);
	ret = ums9117_fb_lcm_wait_idle(ufb);
	if (ret)
		return ret;
	writel(ufb->lcm_timing, ufb->lcm + UMS9117_FB_LCM_CS0_TIMING);
	return ums9117_fb_lcm_wait_idle(ufb);
}
...
int ums9117_fb_lcm_begin_frame(struct ums9117_fb *ufb)
{
	...
	if (ret)
		return ret;
	writew(MIPI_DCS_WRITE_MEMORY_START, ufb->lcm_command);
	ret = ums9117_fb_lcm_wait_idle(ufb);
	if (!ret)
		writel(0x28, ufb->lcm + UMS9117_FB_LCM_CS0_MODE);
	return ret;
}
...
int ums9117_fb_lcm_init_transport(struct ums9117_fb *ufb,
				  struct platform_device *pdev)
{
	return ums9117_fb_lcm_init(ufb, pdev);
}

int ums9117_fb_lcm_enable_transport(struct ums9117_fb *ufb)
{
	return ums9117_fb_lcm_enable(ufb);
}
