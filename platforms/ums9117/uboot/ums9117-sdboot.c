...
#include <fplinux-uboot-target.h>

#define UMS9117_SDBOOT_FIT_PATH "FPLINUX.ITB"
#define UMS9117_SDBOOT_CONFIG FPLINUX_UBOOT_TARGET
#define UMS9117_SDBOOT_KERNEL "kernel"
#define UMS9117_SDBOOT_FDT "fdt"
#define UMS9117_SDBOOT_VERIFY_VALUE "yes"
			  ...
			  uint32_t *fit_bytes)
{
	...
	if (fs_set_blk_dev("mmc", "0:1", FS_TYPE_FAT) ||
	    fs_size(UMS9117_SDBOOT_FIT_PATH, &size) || size <= 0)
		return -EINVAL;
	if (size > handoff->fit_limit)
		return -EFBIG;
	if (fs_set_blk_dev("mmc", "0:1", FS_TYPE_FAT) ||
	    fs_read(UMS9117_SDBOOT_FIT_PATH, handoff->fit_phys, 0, size,
		    &actual) ||
	    actual != size)
		return -EIO;
	*fit_bytes = size;
				   ...
				   const struct bootm_headers *images)
{
	...
	if (!images->fit_uname_cfg ||
	    strcmp(images->fit_uname_cfg, UMS9117_SDBOOT_CONFIG) ||
	    !images->fit_uname_os ||
	    strcmp(images->fit_uname_os, UMS9117_SDBOOT_KERNEL) ||
	...
	if (kernel != images->fit_noffset_os || fdt < 0 ||
	    strcmp(fit_get_name(fit, kernel, NULL), UMS9117_SDBOOT_KERNEL) ||
	    strcmp(fit_get_name(fit, fdt, NULL), UMS9117_SDBOOT_FDT) ||
		...
		return -EINVAL;
	/* FINDOTHER checks the FDT header after copying to its aligned load address. */
	if (fit_image_get_type(fit, fdt, &type) ||
	    ...
	    fdt_totalsize(fdt_data) != fdt_size || !has_sha256_hash(fit, fdt))
		return -EINVAL;
	return 0;
}
				    ...
				    uint32_t *zimage_bytes)
{
	...
	if (!images->fit_uname_cfg ||
	    ...
	    strcmp(images->fit_uname_fdt, UMS9117_SDBOOT_FDT) ||
		       ...
		       uint32_t *dtb_bytes)
{
	...
	if (env_set("verify", UMS9117_SDBOOT_VERIFY_VALUE))
		return -EIO;
	length = snprintf(fit_spec, sizeof(fit_spec), "%x#%s",
			  handoff->fit_phys, UMS9117_SDBOOT_CONFIG);
...
static int do_ums9117_sdboot(struct cmd_tbl *cmdtp, int flag, int argc,
			     char *const argv[])
{
...
U_BOOT_CMD(sdboot, 1, 0, do_ums9117_sdboot,
	   "verify FPLINUX.ITB and boot Linux from the UMS9117 microSD", "");
